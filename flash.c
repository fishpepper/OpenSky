...
void flash_write(uint16_t address, uint8_t *data, uint16_t len){
    ...
    debug("flash: will write page at 0x"); debug_flush();
    ...
    debug("flash: will write ["); debug_flush();
    i=0;
    while (i < len){
    ...
    debug("flash: will write flash now\n"); debug_flush();
